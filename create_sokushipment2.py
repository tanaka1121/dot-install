...
def build_prep_sheet(wb):
    ...
    mg(ws, 2, 2, 2, 13, "作業項目: 【即出荷】事前準備", FONT_HEAD, AL_L_MID)
    mg(ws, 3, 2, 5, 13,
       "この準備が完了したら【即出荷】(2) シートへ。\n"
       "・棚付けをする場合 → 作業項目【棚付け】→ 手順【1】へ\n"
       "・即出荷をする場合 → 準備完了後、【即出荷】(2) 手順【1】へ",
    ...
    build_step(
        ws, row, "1",
        "タブレットD（通過検品_即出荷）の電源をONにし、スキャナの動作を確認する",
        "WMSアプリが起動しているか確認する。\n"
        "スキャナでバーコードをテストスキャンし、正常に読み取れるか確認する。",
    )
    row += 12

    build_step(
        ws, row, "2",
        "梱包資材の在庫を確認し、作業台に準備する",
        "確認する資材:\n"
        "・出荷箱（各サイズ: No.6・No.8・S18 等）\n"
        "・クラフトテープ / OPPテープ\n"
        "・緩衝材（エアーキャップ等）\n"
        "不足している場合はリーダーへ連絡すること。",
    )
    row += 12

    build_step(
        ws, row, "3",
        "「納品書在中」スタンプとスタンプ台を準備する",
        "スタンプ台のインク残量を確認する。\n"
        "薄い場合は補充またはリーダーへ連絡すること。",
    )
    row += 12

    build_step(
        ws, row, "4",
        "工程内不良カードの場所を確認し、作業エリアを決定する",
        "工程内不良カードが所定の棚にあるか確認する。\n"
        "（詳細: 【oLPN分割】シート参照）\n\n"
        "作業エリアの決定:\n"
        "・HP側の場合    → ドーリー不要\n"
        "・転送側の場合  → ドーリーを事前に作業台横に準備する",
    )
...
def build_main_sheet(wb):
    ...
    build_step(
        ...
        q_reason="緩衝材不足→破損 / 赤丸なし→数量確認漏れ防止のため",
    )
    row += 12
    ...
    # ── 手順6: ラベルを確認する（梱包後・赤丸後）
    build_step(
        ws, row, "6",
        "ラベルを確認する",
        "梱包が完了し赤丸をつけた後、ラベルの内容を確認する。\n"
        "・発送先・品番・数量がピックラベルと一致しているか\n"
        "・部材の場合、1つのオリコンに複数ラベルが貼ってあるので確認漏れに注意",
        q_text="ラベルの発送先・品番・数量の確認漏れ厳禁",
        q_reason="梱包後のラベル照合が誤出荷を防ぐ最後の砦となるため",
    )
    row += 12

    # ── 手順7: バッチを確認する（スキャン→発行確認）
    build_step(
        ws, row, "7",
        "タブレットD（通過検品_即出荷）のスキャナで\nG#（OLPN）ラベルをスキャンし、バッチを確認する",
        "スキャン後、以下が発行される:\n"
        "①納品書   → 残1梱包のみ出力（複数枚出ることあり）\n"
        "②送り状   → シューター番号を納品書と照合\n"
        "③ランクラベル → ランクオーダーのみ出力\n\n"
        "20kg以上・S18箱のみの商品は配送方法を Q/T → S（佐川）へ切り替えること。",
        q_text="納品書と送り状のシューター番号が一致しているか確認",
        q_reason="印刷機トラブル等で直前の別注文と混在しないため（誤配送防止）",
    )
    row += 12
